...
import { Alert, Button, Col, List, Menu, Row, Spin } from "antd";
...
function RollDice({ rolledNumber }) {
  ...
  function rollDiceWithoutValues() {
    ...
    if (typeof rolledNumber === "number") {
      let firstRollNumber, secondRollNumber, values;
      if (rolledNumber > 8) {
        firstRollNumber = Math.round(rolledNumber / 2);
        secondRollNumber = rolledNumber - firstRollNumber;
        values = [
          firstRollNumber - 4 === 0 ? 1 : firstRollNumber - 4,
          4,
          secondRollNumber - 4 === 0 ? 1 : secondRollNumber - 4,
          4,
        ];
      }
      ...
      if (rolledNumber <= 8) {
        firstRollNumber = Math.round(rolledNumber / 2);
        firstRollNumber = firstRollNumber === 0 ? 1 : firstRollNumber;
        secondRollNumber = rolledNumber - firstRollNumber;
        secondRollNumber = secondRollNumber === 0 ? 1 : secondRollNumber;
        values = [firstRollNumber, secondRollNumber];
      }
      ...
      const numberOfDice = values.length;
      const options = {
        element,
        numberOfDice,
        callback: response,
  ...
  return (
    <>
      <div>
        {/* <button onClick={rollDiceWithoutValues}>roll</button> */}
        <div
          style={{ border: "2px solid #dce0e0", background: "#e8eded", height: 200, padding: 40 }}
          id="dice-box1"
        ></div>
      </div>
    </>
...
function App(props) {
  ...
  */
  ...
  const [rolledNumber, setRolledNumber] = useState(undefined);
  ...
  const withdraw = async () => {
    tx(writeContracts.RiggedRoll.withdraw({ gasLimit: 500000 }), update => {
      console.log("TX UPDATE", update);
    });
  };

  const riggedFilter = readContracts.RiggedRoll?.filters.Roll(riggedRoll.address, null);

  readContracts.RiggedRoll?.on(riggedFilter, (_, value) => {
    if (value) {
      setRolling(false);
      ...
      setRolledNumber(value.toNumber());
      setRolling(false);
    }
  });
              ...
              <div id="centerWrapper" style={{ padding: 16 }}>
                ...
                <div style={{ padding: 16, format: "flex", flexDirection: "row" }}>
                  ...
                  <div style={{ padding: 16 }}>
                    ...
                    <Button style={{ margin: 16 }} type="primary" danger disabled={diceRolled} onClick={withdraw}>
                      Hack money!
                    </Button>
                  </div>
                </div>
                ...
                <RollDice rolledNumber={rolledNumber} />
              </div>
